...
class Scratch: 
  def __init__(self, f_saved): 
    ...
    if check_if_file_exists(f_saved): 
      ...
      ##### add features
      self.features = [(" ".join(x.split("Feature_")[-1].split("_")), scratch_load[x]) for x in scratch_load.keys() if "Feature_" in x]
      self.interview_info = scratch_load["interview_info"] 
      self.interact_info = scratch_load["interact_info"]
      self.offer_info = scratch_load["offer_info"]
      self.interview_counter = scratch_load["interview_counter"]
      self.interact_counter = scratch_load["interact_counter"]
      self.can_hire = scratch_load["can_hire"]
      self.can_be_hired = scratch_load["can_be_hired"]
      self.job_details = None
      self.sim_nr = scratch_load["sim_nr"]

      self.group_identity = scratch_load["Feature_Group_Identity"][0]
      self.group_condition = scratch_load["group_condition"]
      self.group_identity_text = scratch_load["group_identity_text"]
      self.threat_text = scratch_load["threat_text"]
  ...
  def get_str_iss(self): 
    ...
    if self.group_condition in [1,2,4,6]: # conditions with groups
      commonset += f"Group Identity: {self.group_identity_text}\n"
    if self.group_condition in [1,2,3,4,6]: # conditions with threats (symbolic, material, non-group threats)
      commonset += f"Additional information: {self.threat_text}\n"
